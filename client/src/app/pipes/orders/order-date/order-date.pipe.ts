...
export class OrderDatePipe implements PipeTransform {
  ...
  transform(orders: FetchOrdersDto[], date: any): any {
    ...
    if (!isStartDateValid && !isEndDateValid) {
      ...
      return orders.filter(order => new Date(order.createdAt).getTime() > startDate.getTime())
    }
    else if (!isStartDateValid && isEndDateValid) {
      return orders.filter(order => new Date(order.createdAt).getTime() < endDate.getTime())
    }
    else {
      if (startDate.getTime() < endDate.getTime()) {
        return orders.filter(order => new Date(order.createdAt).getTime() < endDate.getTime() && new Date(order.createdAt).getTime() > startDate.getTime())
      }
      else {
        return []
      }
